...
class FoodListBloc {
  FoodListBloc() {
    if(_date.valueOrNull == null && MemoryApp.selectedDate == null) _date.value = DateTime.now().toString().substring(0, 10);
    else if(_date.valueOrNull == null && MemoryApp.selectedDate != null) _date.value = MemoryApp.selectedDate!;
    debugPrint('bloc foodlist ${_date.value}');
    _loadContent(invalidate: true);
  }
  ...
  void _loadContent({bool invalidate = false}) {
    _loadingContent.value = true;
    _repository
        .foodList(
            _date.value, invalidate: invalidate)
        .then((value) {
  ...
  void setTheme(){
    debugPrint('theme ${_foodList.value?.isFasting} / ${_foodList.value?.isFasting == boolean.True}');
    if(_foodList.value?.isFasting == boolean.True) _appBloc.changeTheme(ThemeAppColor.DARK);
    else _appBloc.changeTheme(ThemeAppColor.DEFAULT);
  }
  ...
  void fillWeekDays() {
    ...
    data.add(WeekDay(gregorianDate: gregorianDate, jalaliDate: jalaliDate, isSelected: gregorianDate.toString().substring(0, 10) == _date.value));
    ...
    for (int i = 1; i < 7; i++) {
      ...
      isSelected: gregorianDate.add(Duration(days: i)).toString().substring(0, 10) == _date.value));
    }
  ...
  void changeDateWithString(String newDate) {
    debugPrint('change date 1 $newDate / ${_previousWeekDay.gregorianDate} / ${_weekDays.value!.length}');
