...
class _MessageTemplateState extends State<MessageTemplate> {
 ...
 initState(){
  ...
  if (templateItem![index].media!.length == 0) {
    return Html(
      data: "<div style='direction:rtl'> ${templateItem[index]
          .alterText} </div>",
      shrinkWrap: true,
    ...
    if (templateItem[index].media!.isNotEmpty && templateItem[index].media![0].mediumType == MediumType.file &&
              ...
              child: Row(
                children: [
                  GestureDetector(
                    ...
                    onTap: () async{
                      debugPrint("path:${tempDir.absolute.path}");
                      debugPrint("path:${templateItem[index].media![0].mediumUrls!.url!}");
                      if (!templateItem[index].media![0].mediumUrls!.url!
                          .contains("${tempDir.path}")) {
                        setState(() => templateItem[index].media![0].progress = true);
                        downloadFile(templateItem[index].media![0].fileName,
                            templateItem[index].media![0].mediumUrls!.url, templateItem[index].media![0]);
                      }
                      else {
                        var f = await OpenFile.open(templateItem[index].media![0].mediumUrls!.url!);
